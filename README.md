# KUIT 9주차 미션 - 동아리 관리 REST API 설계 및 구축

## 📋 프로젝트 개요

이번 9주차 미션은 SpringBoot 를 사용하여 동아리 관리 REST API 를 설계 및 구축해보는 미션입니다.
...
- **토큰**: 필요 
- **기능**: 현재 로그인한 학생이 가입한 동아리 목록 조회 - 학생 이름과 동아리 이름까지 모두 반환되도록 구현
- **응답 형식**
  ```json
  {
    "isSuccess": true,
...
4. **무한 스크롤**: TODO 1에서는 `lastId`를 기준으로 다음 페이지를 조회하는 방식으로 구현해야합니다. `lastId` 는 이번 페이지에 보여질 가장 마지막 데이터의 clubId 를 의미합니다.
